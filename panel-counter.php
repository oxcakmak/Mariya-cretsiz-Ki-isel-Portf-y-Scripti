			...
			<div class="table-responsive" style="border:1px solid #efefef;">
				<table class="table align-items-center table-flush">
					...
					foreach($dbh->get("counter") as $counterRow){ $i++; echo '
						<tr id="'.$counterRow['counter_slug'].'">
							<td>'.$i.'</a></td>
							<td>'.$counterRow['counter_title'].'</td>
							<td>'.$counterRow['counter_number'].'</td>
							<td><button class="btn btn-sm btn-danger deleteCounter" onclick="deleteCounter(\''.$counterRow['counter_slug'].'\')">'.$lang['t_remove'].'</button></td>
						</tr>
					'; }
					echo '</tbody>
				</table>
...
function deleteCounter(hash){
	...
	$.ajax({
		type: "POST",
		url: "'.POST_URL.'",
		data: {counterSlug:hash, actionDeleteCounter:"actionDeleteCounter"},
...
$(".actionAddCounter").click(function(e){
	...
	$.ajax({
		type: "POST",
		url: "'.POST_URL.'",
		data: {cTitle:$("#mcTitle").val(), cValue:$("#mcValue").val(), actionAddCounter:"actionAddCounter"},
		success: function(response){
